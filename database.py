...
def create_tables():
    ...
    try:
        ...
        db_cursor.execute("CREATE TABLE rules (rule_id INT AUTO_INCREMENT PRIMARY KEY, rule_name VARCHAR(255) NOT NULL, description TEXT, category_id INT NOT NULL, approved_by VARCHAR(255), approved_at DATETIME, rule_metadata JSON, FOREIGN KEY (category_id) REFERENCES categories(category_id));")
        ...
        db_cursor.execute("CREATE TABLE audit_logs (log_id INT AUTO_INCREMENT PRIMARY KEY, entity_type VARCHAR(255), entity_id INT, action VARCHAR(255), actor VARCHAR(255), timestamp_at DATETIME DEFAULT CURRENT_TIMESTAMP, delta JSON);")
