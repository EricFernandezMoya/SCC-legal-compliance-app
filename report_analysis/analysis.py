...
from database.database import create_db_server_connection, selectContractsByGroup
...
def fetch_group_documents(group_id, conn=None):
    ...
    """
    rows = selectContractsByGroup(group_id)
    ...
    for row in rows:
        if row[6]:
            ...
            file_path = row[7]
        name = row[3] or os.path.basename(file_path)
