...
def _fetch_db_rule_id(artifact_rule_name):
    """
    Resolve an artifact rule id (e.g. 'R1') to the DB rules.rule_id integer.
    Ingestion stores the artifact id in rules.rule_name.
    """
    rows = selectRuleIdByName(artifact_rule_name)
    if not rows:
        return None
    return rows[0][0]


def save_report(contract_id, findings, prior_report_id=None, is_group=0):
    """
    ...
    insertComplianceReport(contract_id, lastSnapshot_id, prior_report_id, None, is_group)
    report_id = selectComplianceReportByContractId(contract_id, is_group)[0][0]
    ...
    for finding in findings:
        ...
        print("risk_level_id: ", risk_level_id)
